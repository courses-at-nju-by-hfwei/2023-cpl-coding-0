...
int main() {
  ...
  const int musician_score_table[NUM_OF_MUSICIANS][NUM_OF_SCORES] = {
      { 0, 10, 20 },
      { 10, 20, 30 },
      { 20, 30, 40 },
      { 30, 40, 50 },
  };
  Print(musician_score_table, NUM_OF_MUSICIANS);

  // TODO: dynamically allocate memory for scores
  int rows = 0;
  ...
  // int row = i / NUM_OF_SCORES;
  // int col = i % NUM_OF_SCORES;
...
// (2) int table[]: int *table
// (2) int table[][COLS] is equivalent to int (*table)[COLS]
...
void Print(const int table[][NUM_OF_SCORES], int num_of_musicians) {
  ...
  printf("table: %p\n\n\n", table);
  for (int i = 0; i < num_of_musicians; i++) {
    int **ptr_table_i = table + i;
    printf("table + %d: %p\n", i, table + i);
    int *ptr_row_i = *(table + i);
    printf("*(table + %d): %p\n\n", i, *(table + i));
